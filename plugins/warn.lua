...
----------------------------------
  ...
  return send_msg(receiver, 'You Cant not warn moderators', ok_cb, false) end
--endif--
  if is_admin2(msg.id) then return send_msg(receiver, 'You Cant not warn admins', ok_cb, false) end
--endif--
  if value then
   if value == '1' then
  ...
  text = 'User [ '..name..' ] Kicked From Group!'
...
--endif--
  if value then
  redis:hdel(hash, msg.id, '0')
  text = 'User ('..msg.id..') Unwarned!'
  else
   text = 'This user have not any warning'
  end
  send_msg(receiver, text, ok_cb, false)
  else
   send_msg(receiver, 'Username not found!', ok_cb, false)
...
local function run(msg, matches)
 ...
 --endif--
 if not is_momod(msg) then return 'You Are not moderator' end
 --endif--
 ----------------------------------
 if matches[1]:lower() == 'warn' and not matches[2] then -- (on reply) /warn
  ...
  else return 'Use reply or username for warning' end
 --endif--
 end
 if matches[1]:lower() == 'warn' and matches[2] then -- /warn <@username>
   if string.match(user, '^%d+$') then
      return 'Use reply or username for warning'
