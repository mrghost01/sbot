...
local function tophoto(msg, success, result)
  ...
  if success then
    local file = './data/sticker/'..msg.from.peer_id..'.webp'
...
local function tosticker(msg, success, result)
  local receiver = get_receiver(msg)
  if success then
    local photofile = './data/sticker/'..msg.from.peer_id..'.jpg'
    print('File downloaded to:', result)
...
local function topng(msg, success, result)
  local receiver = get_receiver(msg)
  if success then
    local photofile = './data/sticker/'..msg.from.peer_id..'.png'
    print('File downloaded to:', result)
    os.rename(result, photofile)
    print('File moved to:', photofile)
    send_document(get_receiver(msg), photofile, ok_cb, false)
...
local function run(msg,matches)
    ...
    if msg.reply_id then
       if msg.to.type == 'document' and redis:get("sticker:photo") then
        if redis:set("sticker:photo", "waiting"..msg.from.id) then
        end
       end
