do
local Mohammad = 179071599 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = 'None'
    ...
    if not value then
	 if result.id == tonumber(Mohammad) then
  ...
  text = text..'Messages Count : '..user_info_msgs..'\n\n'
  text = text..''
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  send_msg(extra.receiver, 'User ID not found!', ok_cb, false)
  end
end
  ...
  local text = 'Full Name : '..(result.from.first_name or '----')..' '..(result.from.last_name or '----')..'\n'
	...
	local hash = 'Rank : '..result.to.id..':variables'
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.peer_id == tonumber(Mohammad) then
			   text = text..'Rank : Sudo (Executive Admin) \n\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = text..'Family : '..(msg.from.last_name or '----')..'\n'	
